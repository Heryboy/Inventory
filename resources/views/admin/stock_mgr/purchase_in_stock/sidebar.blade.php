...
<div class="left_col scroll-view">
  ...
  <div id="sidebar-menu" class="main_menu_side hidden-print main_menu">   
    <div class="menu_section">
     ...
     <div class="lbl-cat">
     	<h3 class="lbl-focus active">{!!trans('stock_mgr/purchase_order.entry_vendor_list')!!}</h3>
     </div>
     <ul class="nav side-menu">
      ...
      ?>
        <li class="active">
          <a><i class="fa fa-th-large"></i> {!!trans('stock_mgr/purchase_order.entry_vendor')!!} <span class="fa fa-chevron-down"></span></a>
          <ul class="nav child_menu" style="display: block;">
            @foreach($getData_arr as $getData)
              <li><a href="{{url('admin/stock_mgr/purchase_order')}}?vendor_id={{$getData['id']}}">{{$getData['name']}}</a></li>
              <?php 
                $i++;
              ?>
            @endforeach
