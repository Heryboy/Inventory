    ...
    <div class="menu_section">
     ...
     <ul class="nav side-menu">
      ...
      @foreach($getData_arr as $getData)
        <li class="active">
          <a><i class="fa fa-th-large"></i> {{$getData['item_category_name']}} <span class="fa fa-chevron-down"></span></a>
          <ul class="nav child_menu" style="display: block;">
            @foreach($getData['sub_category_arr'] as $item_sub)
              <li><a href="{{url('admin/setup_mgr/item')}}?item_subcatID={{$item_sub['id']}}">{{$item_sub['name']}}</a></li>
            @endforeach
          </ul>
        </li>
        <?php 
