  ...
  <div class="main_container">
    ...
    <div class="right_col" role="main">
      ...
      <div class="row">
        ...
        <div class="col-sm-12">
          @include("admin.discount.discount_item.filter")
        </div>
        {{-- {!! Form::open(['url' => 'admin/discount/discount_item','id'=>'form-discountitem','class'=>'form-horizontal']) !!} --}}
        @if(!isset($DiscountItem))
          {!! Form::open(['url' => 'admin/discount/discount_item','id'=>'form-discountitem','class'=>'form-horizontal']) !!}
        @else
          {!! Form::model($DiscountItem,['method' => 'PATCH','id'=>'form-discountitem','class'=>'form-horizontal','route'=>['admin.discount.discount_item.update',$DiscountItem->id]]) !!}
        @endif
          <div class="well">
            {{-- group-filter --}}
            <div class="row col-sm-12">
              <div class="col-sm-3">
                {!! Form::text('name',null,['class'=>'form-control','placeholder'=>trans("discount/discount_item.name")]) !!}
              </div>
              <div class="col-sm-3">
                {!! Form::text('start_date',null,['class'=>'date-picker form-control','placeholder'=>trans("discount/discount_item.start_date")]) !!}
              </div>
              <div class="col-sm-3">
                {!! Form::text('end_date',null,['class'=>'date-picker form-control','placeholder'=>trans("discount/discount_item.end_date")]) !!}
              </div>
              <div class="col-sm-3">
                {!!trans('discount/discount_item.is_active')!!} {!! Form::checkbox('is_active',null,true,['class' => 'flat form-control']) !!}
              </div>
            </div>
                ...
                <!-- <table id="example1" class="table table-bordered table-striped dataTable"> -->
                  <thead>
                    <tr>
                      ...
                      <th>Item Name</th>
                      <th>Discount</th>
                      <th>Discount Methods</th>
                      <th>#</th>
                    </tr>
                  </thead>
                  <tbody>
                    @if(isset($DiscountItemDetail))
                      @foreach($DiscountItemDetail as $row)
                        <tr id="attr-row{{$row->item_id}}">
                          ...
                          <td>
                            {!! Form::select("discount_method_id[]", $DiscountMethods, $row->discount_method_id, ["id"=>"discount_method_id","class"=>"form-control"]) !!}
                          </td>
                          <td>
                            <center>
                              <button type="button" onClick="initRemoveRow({{$row->item_id}},{{$row->category_id}},'{{$row->ItemCategory->item_category_name}}','{{$row->Item->item_code}}','{{$row->Item->name}}');" class="btn btn-danger btn-xs"><i class="fa fa-minus"></i></button>
                            </center>
                          </td>
                        </tr>
                      @endforeach
  ...
  <script type="text/javascript">
    $(".initRowItem").click(function(){
      ...
      html += '<tr id="attr-row'+itemID+'">';
        ...
        html += '<td>';
        html += '{!! Form::select("discount_method_id[]", $DiscountMethods, null, ["id"=>"discount_method_id","class"=>"form-control"]) !!}';
        html += '</td>';
