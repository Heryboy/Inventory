<script type="text/javascript">
				...
				if(response['data'].length>0){
					...
					$.each(response.data, function(label, data){
						...
						html +='<tr class="'+class_html+'">';
							...
							html +='<td>'+formatCurrency(data['unit_price'],'$')+'</td>';
							html +='<td>'+data['qty']+'</td>';
							html +='<td>'+formatCurrency(data['price'],'$')+'</td>';
						html +='</tr>';
						i++;
					});
				}else{
	...
	$("#initPrintOutInvoice").click(function(){
		var order_id = $("input[name='order_id']").val();
	...
	function initRemoveItem(order_detail_id){
		$('#attribute-item'+order_detail_id+'').remove();
		var order_id = $("input[name='order_id']").val();
	...
	$("#initGenerateOrder").click(function(){
		...
		$.ajax({  
			...
			success: function(response)
			{
				window.location.href="{{url('pos/salepanel/dashboard')}}?order_id="+response;
