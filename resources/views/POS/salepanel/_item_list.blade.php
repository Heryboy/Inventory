					...
					<span class="col-sm-12">
						<div class="cat-list">
							@foreach($ItemCategory as $row)
								<a onclick="initFilterByCategory({{$row->id}});" href="javascript:void(0);" class="initChange" data-id="{{$row->id}}" rel="{{$row->id}}">{{$row->item_category_name}}</a>
							@endforeach
						</div>
					</span>
				</div>
				...
				<div class="sub-category col-sm-2">
					<ul class="subcat-list">
						@foreach($ItemSubCategory as $row)
							<li><a class="subactive" onclick="initFilterBySubCategory({{$row->id}});" href="javascript:void(0);">{{$row->name}}</a></li>
						@endforeach
					</ul>
