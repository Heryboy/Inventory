...
class TransferItemReportController extends Controller
{
    public $view_title = "report/transfer_item.entry_title";
    public $action = "Edit";
    ...
    // sold_qty VARCHAR(120),
      ...
      $transferItems = $this->getForm($start_date,$end_date,$branchid);

      $getData_arr = $this->getData();

      return view('admin.report.transfer_item.index')
    ...
    public function getForm($start_date,$end_date,$branchid){
      ...
      // SET @end_date = '2018-12-25';
      // SET @branch = 1;
      
      $transferItems = DB::select( 
        DB::raw("
        SELECT 
        t.transfer_no,
        t.transfer_date,
        ...
        u.name AS unit,
        u.name AS unit_name
        FROM ".env('DB_PREFIX')."transfer_detail td
        JOIN ".env('DB_PREFIX')."transfer t ON t.id =td.transfer_id
        JOIN ".env('DB_PREFIX')."item i ON i.id = td.item_id
        JOIN ".env('DB_PREFIX')."unit u ON u.id = td.unit_id
        JOIN ".env('DB_PREFIX')."branch fb ON fb.id = t.from_branch_id
        JOIN ".env('DB_PREFIX')."branch tb ON tb.id = t.to_branch_id
        WHERE t.from_branch_id=".$branchid."
        AND DATE_FORMAT(t.transfer_date,'%Y-%m-%d') 
        BETWEEN DATE_FORMAT('".$start_date."','%Y-%m-%d') 
        AND DATE_FORMAT('".$end_date."','%Y-%m-%d')
        GROUP BY td.item_id, td.unit_id
    ...
    public function getPrint(){
      ...
      return view('admin.report.transfer_item.print')->with('Branch',$Branch->branch_name)->with('transferItems',$transferItems);
