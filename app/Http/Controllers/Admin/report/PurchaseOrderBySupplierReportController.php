...
class PurchaseOrderBySupplierReportController extends Controller
{
    ...
    // sold_qty VARCHAR(120),
      ...
      $purchaseOrderBySupplier = $this->getForm($start_date,$end_date,$branchid);

      $getData_arr = $this->getData();

      return view('admin.report.purchase_order_by_supplier.index')
                  ->With('ItemCategory',$ItemCategory)
                  ->with('getData_arr',$getData_arr)
                  ->With('Branch',$Branch)
                  ->with('from_date',$start_date)
                  ->with('to_date',date('Y-m-d'))
                  ->With('purchaseOrderBySupplier', $purchaseOrderBySupplier)
    ...
    public function getForm($start_date,$end_date,$branchid){
      ...
      $purchaseOrderBySupplier = DB::select( 
        DB::raw("
        SELECT 
          s.*,
          i.id AS item_id,
          ...
          JOIN ".env('DB_PREFIX')."supplier s on s.id = po.supplier_id
      ...
      "));
      $data = [];
      if($purchaseOrderBySupplier){
        $data = $purchaseOrderBySupplier;
        return $data;
      }else{
        return $data;
      } 
    ...
    public function getPrint(){
      ...
      return view('admin.report.purchase_order_by_supplier.print')->with('Branch',$Branch->branch_name)->with('purchaseOrderBySupplier',$purchaseOrderBySupplier);
