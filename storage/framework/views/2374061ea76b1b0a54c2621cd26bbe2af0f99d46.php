 
<?php $__env->startSection('content'); ?>
  <!-- header -->
  <?php echo $__env->make('admin.common.header', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
  <form novalidate="" action="#" id="demo-form2" data-parsley-validate="" class="">
    
    <!-- main-container -->
    <div class="main_container">
      <!-- col-md-3 -->
      <div class="col-md-3 left_col">
       <?php echo $__env->make('admin.report.sidebar', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
      </div>
      <!-- top navigation -->
      <div class="top_nav">
         <div class="nav_menu">
           <nav class="" role="navigation">
             <div class="nav toggle" style="margin-bottom:10px;">
               <a id="menu_toggle"><i class="fa fa-bars"></i></a>
             </div>
           </nav>
         </div>
      </div><!-- /top navigation -->
      <!-- page content -->
      <div style="min-height: 650px;" class="right_col" role="main">
        <div class="row">
          <div class="clearfix"></div>
          <div class="col-md-12 col-sm-12 col-xs-12">
            <?php echo $__env->make('admin.common.message', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
            <div class="x_panel">
              <div class="x_title">
                <div class="row">
                  <h2><i class="fa fa-wa fa-flag"></i> <?php echo trans('report/item_information_report.entry_title'); ?></h2>
                  <div class="clearfix"></div>
                </div>
              </div>
              <?php /* filter */ ?>
              <?php echo $__env->make("admin.report.item_information.filter", array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>

              <!-- row -->
              <div class="row">
                <!-- x_content -->
                <div class="x_content">
                  <table id="example1" class="table table-bordered table-striped dataTable no-footer" role="grid" aria-describedby="example1_info">
                    <thead>
                      <tr>
                        <th><input type="checkbox" class="flat" name=""></th>
                        <th><?php echo trans('setup_mgr/item.item_name'); ?></th>
                        <th><?php echo trans('setup_mgr/item.item_barcode'); ?></th>
                        <th><?php echo trans('setup_mgr/item.item_code'); ?></th>
                        <th><?php echo trans('setup_mgr/item.net_price'); ?></th>
                        <th><?php echo trans('setup_mgr/item.cost'); ?></th>
                        <th><?php echo trans('setup_mgr/item.retail_price'); ?></th>
                        <th><?php echo trans('setup_mgr/item.item_category'); ?></th>
                        <th><?php echo trans('setup_mgr/item.item_sub_category'); ?></th>
                        <th><?php echo trans('setup_mgr/item.item_type'); ?></th>
                      </tr>
                    </thead>
                    <tbody>
                    
                    <?php $i = 1;?>
                      <?php foreach($items as $item): ?>
                        <tr>
                          <td><input type="checkbox" class="flat" name=""></td>
                          <td><?php echo e($item->name); ?></td>
                          <td><?php echo e($item->item_barcode); ?></td>
                          <td><?php echo e($item->item_code); ?></td>
                          <td><?php echo e($item->net_price); ?></td>
                          <td><?php echo e($item->cost); ?></td>
                          <td><?php echo e($item->price); ?></td>
                          <td><?php echo e($item->item_cat_name); ?></td>
                          <td><?php echo e($item->item_sub_catName); ?></td>               
                          <td><?php echo e($item->item_type_name); ?></td>         
                        </tr>
                        
                       <?php $i++; ?>

                     <?php endforeach; ?>
                    
                    </tbody>
                    
                  </table>
                </div>
              </div>

            </div>
          </div>
        </div>

        <!-- footer content -->
        <?php echo $__env->make('admin.common.footer', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
        <!-- /footer content -->
      </div>
      <!-- /page content -->
    </div>

  </form>

<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.common.layout', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>