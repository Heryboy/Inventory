  ...
  $.widget('ui.weekCalendar', (function() {
    ...
    return {
      ...
      _renderCalendarHeader: function($calendarContainer) {
        ...
        var _columnBaseClass = 'ui-state-defaudlt wc-dayd-column';
        for (var i = 1; i <= options.daysToShow; i++) {
          if (!showAsSeparatedUser) {
            calendarHeaderHtml += '<td class=\"' + _columnBaseClass + ' wc-day-column-first wc-day-column-last day-' + i + '\">';
            calendarHeaderHtml += '<div class=\"wc-full-height-column wc-day-column-inner day-' + i + '\"></div>';
            ...
            for (var j = 0; j < uLength; j++) {
              ...
              calendarHeaderHtml += '<td class=\"' + _columnBaseClass + ' ' + columnclass + ' day-' + i + '\">';
              calendarHeaderHtml += '<div class=\"wc-full-height-column wc-day-column-inner day-' + i;
              calendarHeaderHtml += ' wc-user-' + self._getUserIdFromIndex(j) + '\">';
        ...
        calendarHeaderHtml += "<table><tr>";
        for (var i = 1; i <= options.daysToShow; i++) {
          calendarHeaderHtml += '<td class=\"wc-day-column-header wc-day-' + i + '\"' + colspan + '></td>';
        }
        calendarHeaderHtml += "</tr></table>";
        calendarHeaderHtml += '<table><tbody><tr>';
        

        for (var i = start; i < end; i++) {
          calendarHeaderHtml += '<td class=\"wc-grid-timeslot-header\">';
          //var bhClass = (options.businessHours.start <= i && options.businessHours.end > i) ? 'ui-state-active wc-business-hours' : 'ui-state-default';
          var bhClass = (options.businessHours.start <= i && options.businessHours.end > i) ? '-ui-state-active' : '-ui-state-default';
          calendarHeaderHtml += '<div class=\"wc-hour-header ' + bhClass + '\">';
          if (options.use24Hour) {
            calendarHeaderHtml += '<div class=\"wc-time-header-cell\">' + self._24HourForIndex(i) + '</div>';
          }
          else {
            calendarHeaderHtml += '<div class=\"wc-time-header-cell\">' + self._hourForIndex(i) + '<span class=\"wc-am-pm\">' + self._amOrPm(i) + '</span></div>';
          }
          calendarHeaderHtml += '</div>';
          calendarHeaderHtml += '</td>';
        }
...
//           calendarHeaderHtml += '</tr>';
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _renderCalendarBodyEvents: function($calendarTableTbody) {
        var self = this, options = this.options, renderRow, showAsSeparatedUser = options.showAsSeparateUsers && options.users && options.users.length,
        start = (options.businessHours.limitDisplay ? options.businessHours.start : 0),
        end = (options.businessHours.limitDisplay ? options.businessHours.end : 24);
